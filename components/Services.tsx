    ...
    <section id={SectionId.SERVICES} className="py-24 relative scroll-mt-20" aria-labelledby="services-title">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-200 max-w-2xl mx-auto font-medium">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {SERVICES.map((service) => (
            <article 
              key={service.id}
              className="bg-slate-900/80 backdrop-blur-md border border-slate-800 p-8 rounded-2xl hover:border-accent-500/50 transition-colors group/card flex flex-col shadow-lg"
            >
              <div className="flex items-start justify-between mb-6">
                <div className="w-16 h-16 bg-slate-800 rounded-xl flex items-center justify-center text-accent-500 group-hover/card:scale-110 transition-transform duration-300 shadow-inner border border-slate-700">
              ...
              <p className="text-slate-200 mb-6 leading-relaxed flex-grow font-medium">
                {service.description}
              </p>
              ...
              <div className="mt-auto pt-6 border-t border-slate-700/50">
                <ul className="space-y-3">
                  {service.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-slate-300 text-sm group/item relative font-medium">
                      <CheckCircle2 size={16} className="text-accent-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <span className="border-b border-dotted border-slate-500 hover:border-accent-400 hover:text-white transition-colors cursor-help" aria-describedby={`tooltip-${service.id}-${idx}`}>
                        {feature.label}
                      </span>
                      
                      {/* Tooltip - High Contrast */}
                      <div id={`tooltip-${service.id}-${idx}`} role="tooltip" className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-2 bg-slate-950 text-xs font-semibold text-white rounded-lg shadow-xl border border-slate-600 w-56 opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-200 z-20 pointer-events-none text-center">
                        {feature.tooltip}
