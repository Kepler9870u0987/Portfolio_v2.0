...
export const Services: React.FC = () => {
  return (
    <div id={SectionId.SERVICES} className="py-24 bg-slate-950/30 backdrop-blur-sm scroll-mt-20">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-4">Servizi & Tecnologie</h2>
              ...
              <div className="mt-auto pt-6 border-t border-slate-800">
                <ul className="space-y-3">
                  {service.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-slate-300 text-sm group/item relative cursor-help">
                      <CheckCircle2 size={16} className="text-accent-500 mt-0.5 shrink-0" />
                      <span className="border-b border-dotted border-slate-600 hover:border-accent-500 transition-colors">
                        {feature.label}
                      </span>
                      
                      {/* Tooltip */}
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 text-xs text-white rounded-lg shadow-xl border border-slate-700 w-56 opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-200 z-10 pointer-events-none text-center">
                        {feature.tooltip}
