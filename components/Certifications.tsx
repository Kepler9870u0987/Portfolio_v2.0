...
const iconMap: Record<string, React.ReactNode> = {
  BrainCircuit: <BrainCircuit size={24} aria-hidden="true" />,
  Globe: <Globe size={24} aria-hidden="true" />,
  Server: <Server size={24} aria-hidden="true" />,
  Lock: <Lock size={24} aria-hidden="true" />,
    ...
    <section id={SectionId.CERTIFICATIONS} className="py-20 bg-slate-950/30 backdrop-blur-sm border-t border-slate-800/50 scroll-mt-20" aria-labelledby="cert-title">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
          <div>
            <h2 id="cert-title" className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3">
              <Award className="text-accent-500" size={32} aria-hidden="true" />
          ...
          <div className="hidden md:block h-px flex-1 bg-slate-800 mx-8" aria-hidden="true"></div>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="p-3 bg-slate-900/50 rounded-lg text-slate-300 group-hover:text-accent-400 transition-colors border border-slate-800">
                  {iconMap[cert.icon] || <Award size={24} aria-hidden="true" />}
                </div>
                <span className="text-xs font-mono text-slate-400 border border-slate-800 px-2 py-1 rounded">
                  {cert.date}
                </span>
              </div>
              ...
              {cert.url && (
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer"
                  aria-label={`Verifica credenziale per ${cert.name}`}
                  className="inline-flex items-center gap-1 text-xs text-accent-500 hover:text-accent-400 font-medium mt-auto focus:outline-none focus:underline"
                >
                  Verifica Credenziale <ExternalLink size={12} aria-hidden="true" />
                </a>
              )}
            </article>
          ))}
